...
import {ToggleSideBar,CloseSideBar,ToggleDevice} from './app'
import {Login,LogOut,FedLogOut,GetUserInfo} from './user'
import {GenerateRoutes,ResetRoutes} from './permission'
import {downfileName} from './download'
import LocalStorage from 'localStorage'
function connectNim ({state, commit, dispatch}, obj) {
  // alert('connectNim')
  let {force, done} = Object.assign({}, obj)
  if (!state.nim || force) {
    console.log(LocalStorage.getItem('uid'), 111)
    console.log(LocalStorage.getItem('sdktoken'), 2222)
    let loginInfo = {
...
export default {
  ...
  */
  ...
  ToggleDevice,
  //user.js
  Login,LogOut,FedLogOut,GetUserInfo,
  
  GenerateRoutes,ResetRoutes,
  downfileName
